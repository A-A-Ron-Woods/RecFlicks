...
from tmdb_api import fetch_movie_data, fetch_random_movie_by_year
...
st.markdown(
    ...
    h1 { 
        ...
        font-size: 100px !important;  /* Large font size */
...
def main():
    ...
    if st.session_state["current_page"] == "home":
        ...
        # Fetch random movie posters for home screen visuals
        if "home_posters" not in st.session_state or st.session_state["last_page"] != "home":
            poster_1980 = fetch_random_movie_by_year(1980)
            poster_2025 = fetch_random_movie_by_year(2025)
            st.session_state["home_posters"] = {"left": poster_1980, "right": poster_2025}

        # Screen layout - poster - Title/text/buttons - poster
        left_col, center_col, right_col = st.columns([1.2, 3, 1.2])

        with left_col:
            if st.session_state["home_posters"]["left"]["poster"]:
                st.markdown(
                    f"""
                    <div style='text-align:center; padding-top: 120px;'>
                        <img src="{st.session_state['home_posters']['left']['poster']}" 
                            style="max-width: 100%; max-height: 600px; border-radius: 10px;">
                        <p style='font-size: 18px; font-weight: bold;'>1980:<br>{st.session_state['home_posters']['left']['title']}</p>
                    </div>
                    """,
                    unsafe_allow_html=True
                )
        ...
        with center_col:
            ...
            st.markdown(
                ...
                'we’ve got your next watch covered.</p>'
                '<p class="custom-text" style="font-size: 22px; color: #bbbbbb;"><em>*Uses TMDb dataset/API*</em></p>',
                unsafe_allow_html=True
            )
            st.markdown(
                "<p style='text-align: center; font-size: 20px;'>Choose how you'd like to get movie recommendations.</p>",
                unsafe_allow_html=True
            )
        ...
        with right_col:
            if st.session_state["home_posters"]["right"]["poster"]:
                st.markdown(
                    f"""
                    <div style='text-align:center; padding-top: 120px;'>
                        <img src="{st.session_state['home_posters']['right']['poster']}" 
                            style="max-width: 100%; max-height: 600px; border-radius: 10px;">
                        <p style='font-size: 18px; font-weight: bold;'>2025:<br>{st.session_state['home_posters']['right']['title']}</p>
                    </div>
                    """,
                    unsafe_allow_html=True
                )
    ...
    # -----------------------Recommend by Genre & Year Page
        ...
        padding_left, center_col, padding_right = st.columns([1, 2, 1])
        with center_col:
    ...
    # ------------------------------Results Page
        ...
        with center_col:
            ...
            with col2:
                st.markdown(
                    f"<div style='text-align: center; font-size: 18px; font-weight: bold;'>Recommendation {current_index + 1} / {len(st.session_state['recommendations'])}</div>",
                    unsafe_allow_html=True
                )
                st.markdown(
